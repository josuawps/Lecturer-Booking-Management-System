...
<div class="container-fluid">
	...
	<div class="card">
		<div class="card-header"><h4>Lecturer Schedule List</h4></div>
			<div class="card-body">
    	...
    	<form method="post" id="appointment_form">
      		<div class="modal-content">
        		...
        		<div class="modal-footer">
          			<input type="hidden" name="hidden_dosen_id" id="hidden_dosen_id" />
          			<input type="hidden" name="hidden_dosen_schedule_id" id="hidden_dosen_schedule_id" />
...
$(document).ready(function(){
	...
	var dataTable = $('#appointment_list_table').DataTable({
		...
		"ajax" : {
			url:"action.php",
			type:"POST",
			data:{action:'fetch_schedule_student'}
		},
		"columnDefs":[
			{
                "targets":[4],				
	...
	$(document).on('click', '.get_appointment', function(){
		...
		var dosen_schedule_id = $(this).data('dosen_schedule_id');
		var dosen_id = $(this).data('dosen_id');

		$.ajax({
			url:"action.php",
			method:"POST",
			data:{action:'make_appointment', dosen_schedule_id:dosen_schedule_id},
			success:function(data)
			{
				$('#appointmentModal').modal('show');
				$('#hidden_dosen_id').val(dosen_id);
				$('#hidden_dosen_schedule_id').val(dosen_schedule_id);
